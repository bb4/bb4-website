...
/**
 * Verify that we correctly evaluate patters on the board.
 *
 * @author Barry Becker
 */
public class TestMoveEvaluation extends TestCase  {
    ...
    public void testOOOEvaluation() {
        ...
        verifyResult(MoveEvaluator.Direction.HORIZONTAL, 2, 3, false, -7960);
        board.undoMove(); // undo move2;

        move2 = TwoPlayerMove.createMove(1, 2,  0, new GamePiece(false));
        board.makeMove(move2);
        verifyResult(MoveEvaluator.Direction.VERTICAL, 3, 2, false, -7960);
        board.undoMove(); // undo move2;

        move2 = TwoPlayerMove.createMove(3, 1,  0, new GamePiece(false));
        board.makeMove(move2);
        verifyResult(MoveEvaluator.Direction.UP_DIAGONAL, 1, 3, false, -7956);
        board.undoMove(); // undo move2; 

         move2 = TwoPlayerMove.createMove(1, 1,  0, new GamePiece(false));
         board.makeMove(move2);
         verifyResult(MoveEvaluator.Direction.DOWN_DIAGONAL, 3, 3, false, -7956);
         board.undoMove(); // undo move2;
    }
    ...
    private void verifyStraightDirectionsResult(int row, int col, boolean player1, int expectedWorth) {
        verifyResult(MoveEvaluator.Direction.HORIZONTAL, row, col, player1, expectedWorth);
        verifyResult(MoveEvaluator.Direction.VERTICAL, col, row, player1, expectedWorth);
    }

    private void verifyDiagonalDirectionsResult(int row, int col, boolean player1, int expectedWorth) {

        verifyResult(MoveEvaluator.Direction.UP_DIAGONAL, col, TicTacToePatterns.WIN_RUN_LENGTH + 1 - col, player1, expectedWorth);
        verifyResult(MoveEvaluator.Direction.DOWN_DIAGONAL, col, col, player1, expectedWorth);
    }
     ...
     */
    private void verifyResult(MoveEvaluator.Direction dir, int row, int col, boolean player1, int expectedWorth) {
