...
/**
 * This class provides static methods to derive the memory footprint of a given object. These methods have only been
 * tested against Sun's jvm prior to jdk 1.4 on a Windows platform. Results may vary with other VM's or other
 * platforms.
 */
public class ObjectSizer {
    ...
    /**
     * computes the size of a given object. does not follow the references that the object contains Limitations: I'm not
     * sure how this works against Array objects.
     *
    ...
    /**
     * Calculates the total memory footprint of a given object. This method traverses the enitre object graph. Objects
     * that are referred to multiple times by the given object (i.e., a reference to an array containing the same object
     * reference) will only be counted once.
     * <p/>
     * LIMITATIONS: This method does NOT subtract out objects in the object graph which are shared with other objects
     * (objects not referencable by the given object). For example, if an immutable String object is refered to by
     * several objects including the target object, the String will still be counted in the memory calculation for the
     * target object. Therefore, if the target object is garbage collected, one cannot gaurantee that the total memory
     * footprint of the target object will be reclaimed.
     *
    ...
    public static int deepSizeOf(Object targetObj, int flags) {
        ...
        List objectStack;    // stack used to traverse the object graph
        ...
        while (!objectStack.isEmpty()) {
            currentObject = objectStack.get(objectStack.size() - 1);
            ...
            if (currentClass.isArray()) {
                classSize = deepSizeOfArray(currentObject, objectStack);
            }
            else {
                classSize = deepSizeOfObject(currentObject, objectStack);
            }
            totalSize += classSize;
        }
        return (totalSize);
     ...
     * @param currentObject
     * @param objectStack
     * @return deep size of Array object
     */
    private static int deepSizeOfArray(Object currentObject, List objectStack) {
        int arrayLength;
        int iArray;           // iterator for the array elements
        arrayLength = Array.getLength(currentObject);
        Class currentClass = currentObject.getClass();
        Class classType = currentClass.getComponentType();

        int classSize = OBJECT_HEADER_SIZE;
     ...
     * @param currentObject
     * @param objectStack
     * @return deep size of non-array object.
     */
    private static int deepSizeOfObject(Object currentObject, List objectStack) {
        ...
        for (; currentClass != null; currentClass = currentClass.getSuperclass()) {
            ...
            if (Sizeable.class.isAssignableFrom((Class) currentClass)) {
                try {
                    ...
                    Object o = null;
                    excludedFields = (String[]) sizeableMethod.invoke(currentClass, o);
