...
class GameTreeViewer extends JPanel implements MouseMotionListener
{
    ...
    private int[] totalAtLevel_ = null;

    private int width_ = 0;
    private int height_ = 0;
    private int levelHeight_ = 0;

    // most recently highlighted path
    private TreePath oldHighlightPath_ = null;
     ...
     */
    public GameTreeViewer( SearchTreeNode root, int depth, ColorMap cmap )
    {
        colormap_ = cmap;
        setRoot(root, depth);
    }

    public void setRoot( SearchTreeNode root, int depth )
    {
        root_ = root;
    ...
    private void initializeTreeStats( SearchTreeNode node, int depth)
    {
        if (node.isLeaf())  {
            if (node.pruned) {
     ...
     */
    public final void highlightPath( TreePath path )
    {
     ...
     * @param g2
     */
    private void drawNode( SearchTreeNode node, int depth, int offset, Graphics2D g2)
    {
        TwoPlayerMove m = (TwoPlayerMove)node.getUserObject();
        g2.setColor( colormap_.getColorForValue(m.value));
     ...
     * @param g2
     */
    private void drawArc( SearchTreeNode parent, SearchTreeNode child, int depth, int offset1, int offset2, Graphics2D g2)
    {
    ...
    protected void paintComponent( Graphics g )
    {
        ...
        width_ = this.getWidth()-2*MARGIN;
        height_ = this.getHeight()-2*MARGIN;
        if (depth_==0)
