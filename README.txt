...
I. Building requires java, svn, intellij, gradle.
 ...
 - Install Ant 1.8+ if you want to deploy from the command line
   (http://ant.apache.org/) migrating away from this toward using gradle exclusively.
...
II. Get the source
    ...
    2. Right click applets dir and select SVN checkout.
       ...
       Alternatively, run "svn co https://free1.projectlocker.com/Tesujisoft/applets/svn/trunk" from new applets dir
    3. The ant tasks rely on foreach which is in ant-contrib.
...
III. Building (3 ways to do it)

otion 1 (gradle)
 - run "gradle" at the root. This will compile all subprojects and run all tests.

option 2 (ant)
 - follow option 1 steps to build everything with gradle.
...
option 3 (intellij)
 ...
 - Follow the option 2 step for building with ant (you do not need to install ant though).
 - Update Project Structure | Project SDK so that the JDK 1.6 points to the install location of your JDK.
 - In Intellij, select SDK and compiler output path to be something like "output"
 ...
 - Gradle tasks can also be run from with intellij
